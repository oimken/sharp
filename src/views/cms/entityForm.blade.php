...
@section('contextbar')
    ...
    <button type="submit" form="sharpform" class="btn navbar-btn btn-ok navbar-right"><i
                class='fa fa-check'></i> {{ trans('sharp::ui.form_updateBtn') }}</button>

    @if(isset($isEmbedded) && $isEmbedded)
        <button type="submit" form="sharp_embedded_cancel" class="btn navbar-btn btn-cancel navbar-right"><i
                    class="fa fa-times"></i> {{ trans('sharp::ui.form_cancelBtn') }}</button>
    @else
        <a href="{{ URL::route("cms.list", [$category->key, $entity->key]) }}"
           class="btn navbar-btn btn-cancel navbar-right"><i
                    class="fa fa-times"></i> {{ trans('sharp::ui.form_cancelBtn') }}</a>
    @endif
...
@section('content')
    ...
    @endif
    @if(isset($instance->__sharp_duplication) && $instance->__sharp_duplication)
        {!! Form::hidden('lang', $instance->__sharp_duplication) !!}
    @endif
            ...
            <div class="col-xs-12">
                <ul class="nav nav-pills entity-tabs" role="tablist">
                    ...
                    @foreach($entity->form_layout as $keytab)
                        <li class="{{ $k==0?'active':'' }}"><a
                                    href="#tab{{ $k++ }}">{{ $entity->form_layout->$keytab->tab }}</a></li>
                    @endforeach
                </ul>
            </div>
        </div>
